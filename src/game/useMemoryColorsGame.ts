...
export function useMemoryColorsGame(config: ClassicGameConfig) {
  ...
  /** Row indices where tile colors are still visible after a guess. */
  const [visibleFeedbackRows, setVisibleFeedbackRows] = useState<Set<number>>(new Set())
  ...
  const newGame = useCallback(() => {
    ...
    setPhase('playing')
    setShake(false)
    setVisibleFeedbackRows(new Set())
  }, [words, wordLength])
  ...
  const submit = useCallback(() => {
    ...
    setGuesses(next)
    setBuffer('')
    setVisibleFeedbackRows((prev) => new Set(prev).add(newIdx))
    window.setTimeout(() => {
      setVisibleFeedbackRows((prev) => {
  ...
  const hideFeedbackForRow = useCallback(
    (rowIndex: number) => {
      if (phase === 'won' || phase === 'lost') return false
      if (rowIndex >= guesses.length) return false
      return !visibleFeedbackRows.has(rowIndex)
    },
    [guesses.length, phase, visibleFeedbackRows],
  )
