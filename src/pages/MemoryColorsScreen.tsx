...
import { useMemoryColorsGame } from '../game/useMemoryColorsGame'
import { wordsForLength, wordLengthFromVariantId } from '../variants/variantWordLength'
import './ClassicWordleScreen.css'

export default function MemoryColorsScreen() {
  const { variantId = 'memory-colors-5' } = useParams<{ variantId: string }>()
  const wordLength = wordLengthFromVariantId(variantId)
  const words = wordsForLength(wordLength)
  const game = useMemoryColorsGame({ words, wordLength, maxGuesses: 6 })
  ...
  return (
    <div className="classic-screen">
      <header className="classic-screen-header">
        ...
        <h1 className="classic-screen-title">Memory colors ({wordLength})</h1>
      ...
      <p className="classic-screen-banner">
        Tile colors flash briefly after each guess—remember them. Keyboard hints still update.
      </p>
      ...
      <WordleGrid
        ...
        phase={game.phase}
        shake={game.shake}
        hideFeedbackForRow={game.hideFeedbackForRow}
      />
